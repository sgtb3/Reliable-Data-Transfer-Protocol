            ...
            while (true)
            {
                ...
                if (is_final_seg)
                {
                    while (true)
                    {
                        ...
                        FIN_sent = true;
                        println(TAG.SEND + "  : (FINAL) seq# --> " + curr_seq_num);
                    ...
                    {
                        try {
                            println(TAG.SEND + "  : |====> MAXIMUM WINDOW SIZE <====|");
                            sleep(timer.timeout);
                        } catch (InterruptedException ignored) {}
                        ...
                        if (timer.timeout_event) {
                            ...
                            println(TAG.SEND + "  : Detected timeout_event. " +
                                    "Resending all packets starting with base --> seq# " + base);
        ...
        public void run() {
            ...
            while (true)
            {
                ...
                if (recv_seg == null)
                {
                    ...
                    if ((curr_ack == prev_ack) && (++overflow_count > MAX_OVERFLOW)) {
                        kill("ERROR: Network error - Packet overflow.");
                    }
                    continue;
                }
                ...
                if (curr_ack == prev_ack)
                {
                    ...
                    if (recv_seg.ack_count >= 3)
                    {
                        ...
                        // if timer is not still running for a retransmitted ACK
                        if ((prev_ack_retrans != curr_ack) && !timer.running || (timer.seq_num != curr_ack))
                        {
                            println(TAG.RECV + "  : Calling fast_retransmit... ");
                            fast_retransmit(curr_ack);
                            recv_seg.ack_count = 0;// reset the duplicate ack count
                ...
                if (FIN_sent && (curr_ack >= final_ack_num)) {
                    ...
                    println(TAG.RECV + "  : ACK (ack# " + curr_ack + ") doesn't " +
                            "correspond to any transmitted seq_num");
                    continue;
                }
