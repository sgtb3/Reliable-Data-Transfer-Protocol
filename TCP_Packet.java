/*
  This class represents a TCP segment. It contains methods
  to encode and decode a segment and perform a 16-bit
...
public class TCP_Packet {
    ...
    public TCP_Packet(int src_port, int dest_port, int seq_num, int ack_num,
        ...
        try {
            ...
            System.out.println("Error: Failed to encode segment. Cause: " + e.getCause());
        }
    }

    // Updates the header field. value = options + options padding
    ...
    // Creates a packet using pre-filled class fields and returns the byte[] segment
    public byte[] encode() throws Exception {
        try {
            ...
            if ((options.length != 0) || ((data.length % 4) != 0))
            {
                ...
                byte opts_pad = (byte) (4 - Math.ceil((double) (2 + options.length) % 4));
                byte data_pad = (byte) (4 - Math.ceil((double) (data.length) % 4));
                ...
                // make a new segment (includes any additional bytes for padding)
                int seg_size = def_head_len + 2 +
                        options.length + opts_pad + data.length + data_pad;
                ...
                System.arraycopy(options, 0, segment, (def_head_len + 2), options.length);
                System.arraycopy(data, 0, segment, opts_end, data.length);

            } else {

                // update the header length field and initialize a new segment
                segment = new byte[def_head_len + options.length + data.length];
                update_header(options.length);

                // copy options and data into segment
                System.arraycopy(options, 0, segment, def_head_len, options.length);
                System.arraycopy(data, 0, segment, def_head_len + options.length, data.length);
            }
            ...
            byte[] ud = URG ? short_to_bytes((short) urg_data) : new byte[]{0, 0};
            ...
            throw new Exception ("Error: Failed to encode packet. Cause: " + e.getCause());
        }
        return segment;
    }

    // Returns a new oldCopies.TCP_Packet object using data extracted from (p)
    public void extract(byte[] p) throws Exception {
        ...
        // if given buffer is not large enough to contain the minimum 20 bytes of TCP header
    ...
    // Returns the flags as an array of one byte
    ...
    public String toString() {
        return "Packet {\n" +
                ...
                "\t\t\t[Data    ("+ data.length +" bytes) = " + Arrays.toString(data) + "]\n" +
                "\t\t\t[Segment ("+ segment.length +" bytes) = " + Arrays.toString(segment) + "]\n" +
                "\t\t\t}";
    }
